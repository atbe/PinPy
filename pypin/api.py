...
class API(object):
    ...
    def get_similar_pins(self, pin_id, bookmark=None, page_size=100):
        if not self.v3_access_token:
            raise RuntimeError('API v3 token not provided to API client! Cannot use this method (get_similar_pins).')

        api_endpoint = f"{self.host}/v3/pins/{pin_id}/related/pin"
        # print(api_endpoint)
        if bookmark:
            ...
            request_url = "{}?page_size={}&access_token={}".format(api_endpoint, page_size, self.v3_access_token)
            return pypin.BookmarkPagination(API.call(request_url), pin_id, self.get_similar_pins)
